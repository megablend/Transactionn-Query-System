...
public class AggregatorMerchantPayRenderer implements MerchantPaymentRenderer {
    ...
    public ByteArrayOutputStream download(DownloadType downloadType, List<AjaxMcashTransaction> transactions) {
        try {
            ...
            throw new RuntimeException("could not generate merchant pay report", e);
        }
    }

    @Override
    public List<String> getTableHeader() {
        return Arrays.asList("MERCHANT", "MERCHANT CODE",
                "TELCO", "USSD AGGR.",
                "SOURCE BANK", "SESSION ID", "REFERENCE CODE",
                "PAYMENT REF.",
                "PHONE NO.",
                "AMOUNT (N)",
